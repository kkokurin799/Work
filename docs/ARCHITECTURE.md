...
Приложение — один процесс на TypeScript. В нём веб-интерфейс, HTTP API, приём webhook, разбор текста и фоновые проверки. Файл SQLite — единственное хранилище: записи, сырые входящие, журнал доставок.
...
| База | SQLite, встроенный в Node `node:sqlite` | Один файл на диске, без отдельного сервера базы. Запись сериализуется внутри процесса |
...
Redis, брокер очередей и отдельный worker-процесс не вводятся. Фоновый цикл живёт в том же приложении и держит блокировку в памяти процесса, чтобы два цикла не отправили сводку дважды. Второй процесс на тот же файл не рассчитан: писатель один.
...
- `id`, `name`, `archived_at`
- уникальность имени: индекс по `lower(trim(name))`
...
Сопоставление имён справочников — точное, по `lower(trim(name))`. Ближайшие имена можно показать в тексте ошибки как подсказку, но провести запись на «похожее» имя нельзя.
...
Один процесс Node.js. База — файл `SQLITE_PATH` (по умолчанию `data/work.db`). На сервере это тот же процесс, диск файла — том сервера. Docker для базы не нужен.

Рядом по расписанию согласованная копия файла (`VACUUM INTO`) раз в сутки, 14 последних копий на этом диске и копия на второй носитель или в объектное хранилище. Восстановление проверяется сценарием А13: подложить файл копии, прочитать проект, задачу и сырое входящее.
...
| Два экземпляра приложения | Не поддерживается: файл открывает один процесс. Внутри процесса цикл оповещений пропускается, если предыдущий ещё идёт |
...
| NFR-2–NFR-5, NFR-7 | HTTPS, секреты в окружении, Argon2id, суточная копия файла SQLite, идемпотентность webhook |
